                ...
                if not skipactualzeropoint:
                    ...
                    if self.forcedxstar is None:
                        ...
                        for fimfile, fnoisefile, fpsffile, fband, fj in \
                            ...
                            print len(snparams.fake_truemag[float(snparams.fake_truemag)<23.])
                            asdf
                            if float(snparams.fake_truemag[fj]) < 23.:
                                ...
                                try:
                                    ...
                                    print 'passing'
                                    continue
                                coords = zip(*fw.wcs_world2pix(np.array(zip(starcat.ra, starcat.dec)), 0))
                                fx_star, fy_star = [], []
                                ...
                                fx_star, fy_star = np.array(fx_star), np.array(fy_star)
...
if __name__ == "__main__":
    ...
    try:
        if os.path.exists("default.configtestforced"):
            args = open("default.configtestforced", 'r').read().split()
        else:
            args = sys.argv[1:]
