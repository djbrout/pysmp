...
#            niter = niter+1
        ...
        #print 'goodtest',fnoise[iylo:iyhi+1,ixlo:ixhi+1][fnoise[iylo:iyhi+1,ixlo:ixhi+1] > 0]
        #print 'goodtest2',fnoise[iylo:iyhi+1,ixlo:ixhi+1][fmask[iylo:iyhi+1,ixlo:ixhi+1] == 0]
...
#            t[2,:] = -scale*dvdy
        ...
        noise_stamp = np.zeros([stampsize,stampsize])
        ...
        out_noise_stamp = np.zeros([stampsize,stampsize])
        ...
        out_noise_stamp[cen-imlen:cen+imlen+1,cen-imlen:cen+imlen+1]  = self.orig_noise[iylo:iyhi+1,ixlo:ixhi+1]

        fsub = f[iylo:iyhi+1,ixlo:ixhi+1]
        
        fsub_full = f[iylo:iyhi+1,ixlo:ixhi+1]
        #if  np.abs(f[ self.psfcenter[1], self.psfcenter[0]] - fsub[radius,radius]) > 0.2:
        ...
        if mpfit_or_mcmc == 'mpfit':
            ...
            for h in range(51):
                try:
                    ...
                    chi=100000
                    sharp=100000
                    if returnStamps: return (errmag,chi,niter,scale,iylo,iyhi,ixlo,ixhi, np.zeros([stampsize,stampsize]), np.zeros([stampsize,stampsize]), np.zeros([stampsize,stampsize]), np.zeros([stampsize,stampsize]))
                    else: return(errmag,chi,niter,scale,np.zeros([stampsize,stampsize]))
            ...
            m = mcmc.metropolis_hastings( model = mcmc_model
                                        ...
                                        , useskyerr=useskyerr )
            if analytical == 'No':
                model, uncertainty, history, sims, xhistory, yhistory = m.get_params()
            elif analytical == 'simple':
                scale = m.get_params_analytical_simple()
                return scale
            elif analytical == 'weighted':
                scale = m.get_params_analytical_weighted()
                return scale
...
#            rsq = rsq[good[0],good[1]]
        ...
        if returnStamps:
            ...
            return(errmag,chi,vals.niter,scale,image_stamp)
