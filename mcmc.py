    ...
    def run_d_mc( self ):
        ...
        while self.z_scores_say_keep_going:
            ...
            if self.counter == 1000:
                ...
                self.modelvec[np.array(self.csv)/ len(self.mask[self.mask>0.].ravel())-mn > 4*st + 5]=0.
                self.modelstd[np.array(self.csv)/ len(self.mask[self.mask>0.].ravel())-mn > 4*st + 5]=0.
        ...
        if flags == 0:
            if fitflags == 0:
                if self.model_errors:
                    ...
                    #print chisq,np.max(wmask.ravel()),np.max(self.mask.ravel()),np.max(sims.ravel()),np.max(data.ravel())
                else:
                    if self.comboerr:
