    ...
    for f in smpfiles[:]:
        ...
        if True:
            ...
            #for p in data['PSF_FILE']:
                ...
                # except:
                #print 'excepted',p
            bigdata['fwhm'].append(data['DIFFIM_FLUX']*0 - 999)
...
#    fwhm = fwhm[ww]
    ...
    rect_histx = [left, bottom_h - .14, width+.2, .28]
    ...
    for dos in ['deep','shallow']:
        ...
        #ax4.scatter(fakemag,fresid,alpha=.03,color='black')
            for filt,col in zip(filts,colors):
                ...
                ax, ayrms = dt.binrms(fakemag[ww][abs(d[ww])<3.], d[ww][abs(d[ww])<3.], np.arange(19., 28, .25), .125)
    ...
    if filter == 'all':
        ...
        ax, ayrms = dt.binrms(hostmag[(d < 3.)&(fakemag>50)&((dpmjd>200)|(dpmjd<-40))],
                              d[(d < 3.)&(fakemag>50)&((dpmjd>200)|(dpmjd<-40))], np.arange(20., 28, .5), .5)
        ax3.plot(ax, ayrms, color='blue', label='ALL SNe', linewidth=3)
    ...
    plt.clf()
    ax, ayrms = dt.binrms(hostmag[(abs(d) < 4.)&(fakemag>50)], d[(abs(d) < 4.)&(fakemag>50)], np.arange(19., 28, .5), .5)
    plt.plot(ax, ayrms, color='blue', label='Fake == 99', linewidth=3)
    ax, ayrms = dt.binrms(hostmag[(abs(d) < 4.) & (fakemag < 23)], d[(abs(d) < 4.) & (fakemag < 23)], np.arange(19., 28, .5), .5)
    plt.plot(ax, ayrms, color='green', label='Fake < 23', linewidth=3)
...
def plotstarrms(flux,fluxerr,zpt,catmag,chisq,rmsaddin,sky,skyerr,poisson,indices,ras,decs,fwhm,zptscat,mjd,field,ccd,band,title='',outdir='.'):
    ...
    if load:
        ...
        a = np.load(outdir + '/pltstarvec_movingstars_new.npz')
        pltvecy = a['pltvecy']
...
if __name__ == "__main__":
    ...
    if filter == 'all':
        cd = []
        for filt in ['g','r','i','z']:
            cd.append('/global/cscratch1/sd/dbrout/summary_results_truefake'+deep_or_shallow+'_' + filt + '.npz')
    else:
