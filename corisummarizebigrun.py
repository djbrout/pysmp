...
def plotstarrms(flux,fluxerr,zpt,catmag,chisq,rmsaddin,sky,skyerr,poisson,indices,ras,decs,title='',outdir='.'):
    ...
    starmagerr = (-2.5*np.log10(flux) + 2.5*np.log10(flux+fluxerr)) + (-2.5*np.log10(flux) + 2.5*np.log10(flux+poisson**2))#+ rmsaddin #+ (-2.5*np.log10(flux) + 2.5*np.log10(flux+poisson))**2)**.5
    ...
    for sme, sm, ind, r, d, cm, f, fe in zip(starmagerr, starmag, indices, ras, decs, catmag, flux, fluxerr):
        ...
        if len(starww) > 5.:
            # if repeatability < .3:
            plt.scatter(np.sqrt(fe**2+abs(f)), repeatability, alpha=.3, color='black')
    ...
    for sme, sm, ind, r, d, cm in zip(starmagerr, starmag, indices, ras, decs, catmag):
        ...
        if len(starww) > 4.:
            # if repeatability < .3:
            plt.scatter(cm, sme - repeatability, alpha=.3, color='black')
