...
def grabdata(tmpwriter,resultsdir,cd,filter = 'g',oldformat=False):
    ...
    for f in smpfiles[:]:
        ...
        for imf,fm in zip(data['IMAGE_FILE'],fakemag):
            ...
            cra = np.isclose(dra, dofakera2, atol=3.e-3)
            tdec = data['DEC']
            ddec = np.zeros(len(dofakedec2)) + tdec[0]
            cdec = np.isclose(ddec, dofakedec2, atol=3.e-3)
            ...
            if not len(dofakemag2[www]) > 0:
                ...
                print fm, dofakemag2[www][0], dofaketflux[www][0], dofakeflux[ www][0]
