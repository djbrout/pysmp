...
class metropolis_hastings():
    ...
    def __init__(self
        ...
        self.galstd[:,:,0] = np.sqrt(self.fgal.real)
        self.galstd[:,:,1] = np.sqrt(self.fgal.imag)
        ...
        if flags == 0:
            if fitflags == 0.:
                if self.shiftgalstd>0.:
                    ...
                    galaxy_conv = np.fft.ifft2(fpsf*self.fouriershift(galoffx,galoffy,self.kicked_fgal)).real
                    star_conv = kicked_modelvec * kicked_psfs
                    sims = (star_conv + galaxy_conv + sky) * self.mask
                    if galoffx > 1.:
                        sims+=np.inf
    ...
    def copy_adjusted_image_to_model( self ):
        ...
        self.fgal = copy(self.kicked_fgal)
    ...
    def update_history( self ):
        ...
        if self.compressioncounter % self.compressionfactor == 0:
            ...
            if not self.dontsavegalaxy:
                self.galhistory.append( self.kicked_fgal )
    ...
    def update_unaccepted_history( self ):
        ...
        if self.compressioncounter % self.compressionfactor == 0:
            if not self.dontsavegalaxy:
                self.galhistory.append( self.fgal )
