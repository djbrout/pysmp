...
def go(fakedir,resultsdir,cacheddata,cd,filter,tfield,dostars,deep_or_shallow,isfermigrid=False,real=False):
    ...
    plotsigmaresid(data['Flux'],data['Fluxerr'],data['FakeMag'], data['FitZPT'], data['FakeZPT'],data['HostMag'],
                   ...
                   data['diffimflux'], data['diffimfluxerr'],filter,data['filter'],deep_or_shallow,data['sky'],data['skyerr'],data['sky'],data['imfiles'],data['DPMJD'],data,real=real)#resultsdir)
        ...
        for jd in mmjd:
            ...
            if len(data['FAKEMAG'][w]) == 0:
                ...
                if jd == 0. :
                    ...
                    raw_input('badddd multiple')
            else:


                bigdata['bootfitzpt'].extend([data['ZPT'][w][0]])
                if not real:
                    bigdata['bootfakemag'].extend([data['FAKEMAG'][w][0]])
                    bigdata['bootfakezpt'].extend([fakezpt[w][0]])
                else:
                    bigdata['bootfakemag'].extend([99])
                    bigdata['bootfakezpt'].extend([31.])
                bigdata['boothostmag'].extend([hostmag])
                bigdata['bootfilt'].extend([filt])
                bigdata['bootdpmjd'].extend([data['DPMJD'][w][0]])
...
def plotsigmaresid(flux,fluxerr,fakemag,fitzpt,fakezpt,hostmag,chisqarr,rmsaddin,deep,outdir,zptstd,diffimflux,diffimfluxerr,filter,filterarr,deep_or_shallow,sky,skyerr,fwhm,imfiles,dpmjd,bigdata,real=False):
    ...
    #raw_input('asdf')
    myww = ((np.asarray(bigdata['bootdpmjd']) < -35.) | (np.asarray(bigdata['bootdpmjd']) > 230.)) & (np.asarray(bigdata['bootfakemag']) > 0.) & (np.asarray(bigdata['bootfitzpt']) != 99.) & (np.asarray(bigdata['bootfitzpt']) != 0.) & (np.asarray(bigdata['m100']) != 0) & (np.asarray(bigdata['s100'])!=0.)
    marr = ['m1', 'm2', 'm3', 'm4', 'm5', 'm10', 'm20', 'm50', 'm100']
    sarr = ['s1', 's2', 's3', 's4', 's5', 's10', 's20', 's50', 's100']
    ...
    #print 'after3',bigdata['bootdpmjd']
    #raw_input()
    #raw_input()
    #print flux[0:10]
    #print fakeflux[0:10]
    ...
    #ax4.scatter(fakemag,fresid,alpha=.03,color='black')
        ...
        ax, ayrms = dt.binrms(fakemag[d < 10.], d[d < 10.], np.arange(20., 28, .1), 1.5)
    ...
    if filter == 'all':
        ...
        for filt, col in zip(filts, colors):
            ...
            ax, ayrms = dt.binrms(hostmag[ww][d[ww] < 9.], d[ww][d[ww] < 9.], np.arange(20., 35., .1), 1.5)
            ax3.plot(ax, ayrms, color=col, label=filt + ' band', linewidth=3, alpha=.8)
            ax3.plot(ax, ax * 0 + 1., linestyle='--', color='black', alpha=.8)
            #ax4.plot(axa, aya, linewidth=3, color=col, label=filt + ' band', alpha=.8)
            #ax4.plot(axa, aya + aystd, linewidth=2, color=col, linestyle='--', alpha=.8)
    ...
    if True:
        ...
        for p, m, s in zip(parr, marr, sarr):
            ...
            trms = (bigdata[m] - bootfakeflux) / np.sqrt(bigdata[s]**2+bigdata[m]+bigdata['boothostmag'])
            #print trms
            #ww = (bigdata['m100'] != 0.) & (bigdata['bootfakemag'] < 3000.) & (bigdata['bootfakemag'] > 0.) & (bigdata[m] != -9999 ) & (bigdata['bootfitzpt'] != 99)# & (abs(trms) < 5.)
            #print bigdata[m][ww].shape, fakeflux[ww].shape
            #for t in trms[ww]:
            #    print t
            #raw_input(m)
            ax, ayrms = dt.binrms(bigdata['boothostmag'], trms, np.arange(min(bigdata['boothostmag']), max(bigdata['boothostmag']), .1), .5)
...
if __name__ == "__main__":
    ...
    resultsdir = '/project/projectdirs/dessn/dbrout/simtest/'
