...
class metropolis_hastings():
    ...
    def plotstamps(self):
        ...
        for i in range(self.Nimage):
            ...
            wmask[wmask > 0] = 1
            v = ((self.sims[i] - self.data[i,:,:]) ** 2 * self.mask * wmask / (self.skyerr[epoch,:,:]**2 + (self.sims[i] - self.sky[i]) / self.gain[i] + self.readnoise[i]/self.gain[i])).ravel()  # hardcoded gain, hardcoded readnoise
            # v = np.real(v)
