            ...
            if 'globalstar.npz' in fname:
                ...
                if not fname in zptfiles:
                    try:
                        ...
                        bigdata['starflux'].extend(zptdata['flux_starnormm'])
                        bigdata['starzpt'].extend(zptdata['flux_starnormm']*0. + zptdata['fit_zpt'])
...
def plotstarrms(flux,fluxerr,zpt,catmag,chisq,rmsaddin,sky,skyerr,title=''):
    ...
    ax3.plot(ax, ayrms, color='grey', label='ZPT Scatter and ChisMin Err', linewidth=3,alpha=.7)
    ax3.plot(ax, ax * 0 + 1., linestyle='--', color='black')
