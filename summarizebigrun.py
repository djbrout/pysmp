...
def plotsigmaresid(flux,fluxerr,fakemag,fitzpt,fakezpt,hostmag,chisqarr):
    ...
    ax, ay, aystd = dt.bindata(chisqarr, d, np.arange(0., 100, .001), window=.01)
    ...
    ax, ayrms = dt.binrms(chisqarr, d, np.arange(0., 100, .001), .01)
    # ax3.plot(ax, ayrms, color='blue',label='RMS',linewidth=3)
    ...
    ww = fakemag == 28.5
    ax, ayrms = dt.binrms(chisqarr[ww], d[ww], np.arange(0., 100, .001), .01)
    ax3.plot(ax, ayrms, color='red', label='FakeMag = 99', linewidth=3)
