            ...
            if 'globalstar.npz' in fname:
                ...
                if not fname in zptfiles:
                    try:
                        ...
                        bigdata['starflux'].extend(zptdata['flux_starnorm'])
                        bigdata['starzpt'].extend(zptdata['flux_starnorm']*0. + zptdata['fit_zpt'])
                        bigdata['catmag'].extend(zptdata['cat_mag'])
                        bigdata['chisq'].extend(zptdata['chisqu'])
                        bigdata['starfluxerr'].extend(zptdata['flux_star_std'])

                        cm = zptdata['cat_mag']
                        fs = zptdata['flux_starnorm']
                        zp = zptdata['fit_zpt']
                        ww = (cm < 19.) & (cm > 17.)
                        ...
                        bigdata['rmsaddin'].extend(zptdata['flux_starnorm']*0. + std)
                        #print 'read in ',fname
...
def plotstarrms(flux,fluxerr,zpt,catmag,chisq,rmsaddin,sky,skyerr,title=''):
    ...
    ax3.plot(ax, ayrms, color='blue', label='MCMC Err', linewidth=3,alpha=.9)
