            ...
            if 'globalstar.npz' in fname:
                ...
                if not fname in zptfiles:
                    try:
                        ...
                        bigdata['starflux'].extend(zptdata['flux_starss'])
                        bigdata['starzpt'].extend(zptdata['flux_starss']*0. + zptdata['fit_zpt'])
                        ...
                        for i in range(len(psfs)):
                            ...
                            bigdata['poisson'].append(np.sqrt(np.sum(psfs[i,:,:].ravel()**2*zptdata['flux_starss'][i])))
                            #print zptdata['flux_starnormm'][i],zptdata['flux_star_std'][i],bigdata['poisson'][-1]
                            #raw_input()
                        cm = zptdata['cat_mag']
                        fs = zptdata['flux_starss']
                        zp = zptdata['fit_zpt']
                        ww = (cm < 18.) & (cm > 16.)
                        ...
                        bigdata['rmsaddin'].extend(zptdata['flux_starss']*0. + std)
                        #print 'read in ',fname
