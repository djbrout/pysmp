...
def go(fakedir,resultsdir,cacheddata,cd,isfermigrid=False):
    ...
    if not cacheddata:
        ...
        dostars = True
        if dostars:
            stardata = np.load('/export/scratch0/ps1sn1/data/v10.0/GPC1v3/eventsv1/smpworkspace/PS_TEST20/stardata/stardata.npz')
            plotstarrms(stardata['starflux'], np.sqrt(stardata['starfluxerr'] ** 2), stardata['starzpt'],
                        ...
                        title='',outdir='/export/scratch0/ps1sn1/data/v10.0/GPC1v3/eventsv1/smpworkspace/PS_TEST20/stardata/')
            sys.exit()
    # print data.keys()
    # print len(data['Flux'])
            ...
            if 'zptstardata.npz' in fname:
                ...
                if not fname in zptfiles:
                    ...
                    if True:
                        ...
                        bigdata['rmsaddin'].extend(zptdata['fitflux']*0. + std)
                        #print 'read in ',fname
...
def pltresid(fitflux,diffimflux,fakezpt,fitzpt,outdir):
    ...
    fakezpt = fakezpt[ww]
    fitzpt = fitzpt[ww]
    diffimflux = diffimflux[ww]*10**(.4*(31-30. ))
...
def plotsigma(flux,fluxerr,dflux,dfluxerr,deltapmjd,chisq,outdir):
    ...
    dww = (deltapmjd > -1000.) & (flux != 0.) & (fluxerr != 0.)
    ...
    wwn =(deltapmjd < -40.) | (deltapmjd > 280.)
    dwwn =(ddeltapmjd < -40.) | (ddeltapmjd > 280.)
    ...
    plt.hist(flux[wwn]/fluxerr[wwn],bins=np.arange(-4.25,4,.5),label='SMP',alpha=.4,normed=True)
    plt.hist(dflux[dwwn] / dfluxerr[dwwn], bins=np.arange(-4.25, 4, .5), label='DIFFIMG', alpha=.4,normed=True)
    plt.xlim(-4,4)
    plt.xlabel('Flux/Fluxerr for Epoch_MJD > PeakMJD + 450 or Epoch_MJD < PeakMJD - 40')
    ...
    plt.clf()
    fig = plt.figure(figsize=(15, 10))
    plt.hist(flux[wwn],bins=np.arange(-1050,2000,100),label='SMP',normed=True,alpha=.4)
    plt.hist(dflux[wwn], bins=np.arange(-1050,2000,100), label='DIFFIMG', normed=True,alpha=.4)
    plt.xlim(-1000,1000)
    plt.xlabel('Flux for Epoch_MJD > PeakMJD + 450 or Epoch_MJD < PeakMJD - 40')
    plt.ylabel('Flux')
    plt.legend()
    ...
    plt.xlim(0, 1000)
    plt.xlabel('Flux Uncertainty for Epoch_MJD > PeakMJD + 450 or Epoch_MJD < PeakMJD - 40')
...
def plotsigmaresid(flux,fluxerr,fakemag,fitzpt,fakezpt,hostmag,chisqarr,rmsaddin,outdir):
    ...
    d = (flux - fakeflux) / ((fluxerr**2 - abs(flux)/3.8 + frms**2)**.5*1.0)
...
def plotstarrms(flux,fluxerr,zpt,catmag,rmsaddin,sky,skyerr,title='',outdir=''):
    ...
    plt.clf()
    plt.scatter(catmag,(flux-catflux)/catflux,alpha=.01)
    ax, ay, aystd = dt.bindata(catmag,(flux-catflux)/catflux, np.arange(min(catmag), max(catmag), .1), window=.5,dontrootn=False)
    plt.plot(ax, ay, linewidth=3, color='orange', label='SMP')
    #plt.plot(ax, ay + aystd, linewidth=2, color='orange', linestyle='--', label='SMP')
    #plt.plot(ax, ay - aystd, linewidth=2, color='orange', linestyle='--', label='SMP')
    plt.ylim(-.03,.03)
    ...
    plt.axhline(0,color='white',linestyle='--')
    plt.xlabel('Cat Mag')
    plt.ylabel('(Fit-Cat)/Cat Flux')
    plt.savefig(outdir+title+'catfluxpercentresid.png')
    #chisq = np.nanmean(chisq[abs(d) < 3])
    sys.exit()
    plt.clf()
...
if __name__ == "__main__":
    ...
    resultsdir = '/pnfs/des/scratch/pysmp/smp_02_simnosnnoskyerr'
    resultsdir= './working/'
    resultsdir= '/export/scratch0/ps1sn1/data/v10.0/GPC1v3/eventsv1/smpworkspace/PS_TEST20/'
